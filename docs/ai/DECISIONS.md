...
## ADR-027 — Mappa versione→milestone orientata alla maturità del prodotto, e policy `-SNAPSHOT` su pom.xml/package.json

**Stato:** decisione documentata (policy). Nessuna automazione in pipeline, nessun bump dei manifest attuali eseguito da questa ADR.

**Problema:** due questioni aperte dopo ADR-026:
1. La mappa versione→milestone in `VERSIONING.md` era ordinata secondo l'ordine **cronologico** con cui le milestone erano state effettivamente implementate (Hardening → Commercializzazione → CI/CD+Deploy). Nella pratica l'ordine reale di completamento è stato diverso (CI/CD è arrivata a maturità prima del completamento di Hardening e ben prima di Commercializzazione), rendendo quella mappa non più coerente con lo stato reale del progetto.
2. `backend/pom.xml` e `frontend/package.json` sono entrambi fermi a `1.0.0`, disallineati dall'ultimo tag reale (`v0.2.0`, vedi ADR-026) — serve una convenzione esplicita su cosa i manifest devono contenere tra una release e l'altra, non solo un valore corretto una tantum.

**Decisione:**
- **Mappa versione→milestone riordinata per maturità del prodotto, non per ordine di implementazione:** `v0.3.0` = completamento FASE 1 Hardening, `v0.4.0` = CI/CD + Release completa (Validate/Build Images/Publish Images/Release, già implementate secondo ADR-021/023/025/026), `v0.5.0` = Deployment operativo (Manual Deploy implementato, primo deploy reale verificato, backup/rollback verificati), `v0.6.0` = completamento FASE 2 Commercializzazione, `v1.0.0` = primo rilascio stabile in produzione con cliente reale (regola invariata rispetto alla versione precedente di `VERSIONING.md`). Sostituisce la mappa "cronologica" (CI/CD come `v0.3.0`) proposta e discussa in precedenza in questa stessa sessione, ma non approvata dall'utente.
- **Niente salto di versione verso `0.9.0` o simili:** confermata l'analisi precedente — un salto comunicherebbe una vicinanza a `1.0.0` non ancora reale (Commercializzazione a zero, nessun deploy mai eseguito), a fronte di nessun beneficio concreto.
- **Convenzione `-SNAPSHOT` su entrambi i manifest:** `backend/pom.xml` (`<version>`) e `frontend/package.json` (`version`) riportano sempre `X.Y.Z-SNAPSHOT` tra una release e l'altra, dove `X.Y.Z` è la **prossima milestone pianificata** nella mappa sopra (non un generico incremento di PATCH).
- **Ciclo di vita ad ogni release** (dettagliato in `RELEASE_PROCESS.md`, step 6-7): (1) commit dedicato su `develop` che rimuove `-SNAPSHOT` portando il manifest al valore esatto della release, prima di aprire la PR verso `main`; (2) merge e tag `vX.Y.Z` su `main` (verificato da `verify-tag`, ADR-026, che continua a funzionare invariato: al momento del tag il manifest è già "pulito"); (3) pubblicazione della release (invariato, ADR-026); (4) nuovo commit **su `develop`**, subito dopo, che bumpa il manifest a `X.Y.Z-SNAPSHOT` della prossima milestone pianificata.
- **Nessuna automazione in questa milestone:** i quattro passi del ciclo di vita restano manuali. Non viene introdotto né un Maven Release Plugin, né uno script `npm version`, né un job CI che esegua bump/commit automatici.

**Motivazione:**
- *Maturità vs cronologia:* un numero di versione è un segnale rivolto verso l'esterno (un futuro collaboratore, un cliente enterprise che ispeziona il repository, un investitore) — deve rispondere alla domanda "quanto il prodotto è vicino a una release stabile e commerciale", non "in quale ordine lo sviluppatore ha scelto di affrontare il lavoro". Se la mappa seguisse l'ordine cronologico reale, la milestone di CI/CD (un lavoro importante ma infrastrutturale) finirebbe prima nell'ordine dei numeri rispetto al completamento della sicurezza applicativa (Hardening) — un segnale fuorviante su cosa sia realmente "più maturo" nel prodotto.
- *`-SNAPSHOT` anche su `package.json`, nonostante non sia convenzione nativa npm:* il progetto usa un'unica versione di repository per backend e frontend (decisione preesistente in `VERSIONING.md`, non toccata da questa ADR). Introdurre due convenzioni diverse (`-SNAPSHOT` solo Maven, un altro segnale o nessun segnale lato npm) romperebbe questa unicità voluta e costringerebbe a controllare due file con logiche diverse per rispondere alla stessa domanda ("sono su una release o in sviluppo?").
- *Bump della versione successiva basato sulla prossima milestone pianificata, non un generico PATCH+1:* coerente con la scelta già esistente in `VERSIONING.md` di usare MINOR per ogni salto di maturità in fase `0.x` — usare PATCH per la versione di sviluppo introdurrebbe un'ambiguità (a cosa corrisponde `0.4.1-SNAPSHOT`? non è mappato a nessuna milestone in tabella).
- *Nessuna automazione ora:* automatizzare bump/commit in pipeline (che dovrebbe scrivere su `develop` e/o `main` per conto dell'utente) è un cambiamento con superficie di rischio non banale (commit automatici che modificano branch protetti, gestione di conflitti, credenziali con permessi di scrittura) da introdurre solo dopo aver validato manualmente l'intero ciclo almeno una volta — coerente con il principio del progetto "nessuna complessità inutile, nessun refactoring senza motivazione concreta" (vedi `replit.md`).

**Alternative scartate:**
- **Mappa versione→milestone ordinata cronologicamente** (`v0.3.0` = CI/CD, come proposto e discusso in precedenza in questa sessione): scartata esplicitamente dall'utente in favore dell'ordinamento per maturità del prodotto.
- **Salto diretto a una versione vicina a `1.0.0`** (es. `0.9.0`): scartato, vedi analisi già condivisa — nessun beneficio concreto, rischio di comunicare una vicinanza a `1.0.0` non reale.
- **`package.json` senza suffisso di sviluppo** (lasciarlo "pulito" sempre, affidandosi solo a Git per sapere se si è su una release): scartata per l'asimmetria con `pom.xml` e la perdita del segnale immediato leggendo il solo file.
- **Bump a PATCH+1-SNAPSHOT genericο dopo ogni release** (convenzione Maven più comune in progetti che seguono SemVer stretto): scartata per questo progetto — in fase `0.x` il progetto usa sistematicamente MINOR per ogni salto di maturità, quindi la versione di sviluppo deve puntare alla prossima milestone MINOR pianificata, non a un PATCH arbitrario privo di significato nella roadmap.
- **Automazione immediata del bump in pipeline:** valutata, deferita esplicitamente dall'utente a una milestone futura.

**Conseguenze:**
- `VERSIONING.md` aggiornato: nuova mappa versione→milestone, nuova sezione "Versione di sviluppo (`-SNAPSHOT`)", tabella "Milestone e fasi" riallineata.
- `RELEASE_PROCESS.md` aggiornato: diagramma e step 6-7 includono esplicitamente i due bump di versione (a versione finale, poi a `-SNAPSHOT` successivo).
- **`backend/pom.xml` e `frontend/package.json` non sono stati modificati da questa ADR**: restano a `1.0.0`, disallineati sia dal tag `v0.2.0` sia dalla nuova policy `-SNAPSHOT`. La correzione (bump al valore concordato, es. `0.3.0-SNAPSHOT`) resta un'azione operativa separata, da eseguire come piccola milestone dedicata prima del prossimo tag reale — deliberatamente non inclusa in questa ADR, che è di sola policy.
- Il job `verify-tag` di `release.yml` (ADR-026) non richiede alcuna modifica: la sua verifica di uguaglianza esatta tra manifest e tag resta corretta, perché il manifest è sempre "pulito" (senza `-SNAPSHOT`) nel momento in cui un tag viene effettivamente pushato.
