...
**Debito tecnico consapevole:** `TenantDetailState` è stato introdotto anticipatamente, prima che un secondo tab esistesse davvero, per evitare un refactoring quando P8/P9 arriveranno. Se il numero di tab dovesse restare limitato a due (Info + Moduli) e nessun'altra necessità di stato condiviso dovesse emergere, va rivalutata la sua rimozione a favore di un binding più diretto (es. `@Input()` sui tab passato dalla shell, se nel frattempo il router Angular offrisse un meccanismo nativo per farlo sulle route figlie). Questa nota va riletta a fine P9, quando il numero reale di tab e le loro esigenze di stato saranno note.
